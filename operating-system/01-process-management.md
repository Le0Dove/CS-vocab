...
- **中文**：中止
- **定义**：异常终止进程的执行
- **说明**：通常由进程自身调用 abort() 或接收到无法处理的信号时发生
- **相关**：Terminate（终止）、Exception（异常）
...
- **中文**：批处理
- **定义**：将一组作业收集起来一起处理的计算模式
- **说明**：批处理系统不需要用户交互，适合大型计算任务；作业调度器负责按策略执行作业
- **相关**：Job（作业）、Job Scheduler（作业调度器）
### Blocked / 阻塞
- **音标**：/blɒkt/
- **中文**：阻塞状态（等待状态）
...
- **中文**：突发时间（CPU 突发时间）
- **定义**：进程连续使用 CPU 的时间段
- **说明**：进程的执行由多个 CPU 突发和 I/O 突发交替组成；调度算法需要预测突发时间
- **相关**：CPU Burst（CPU 突发）、I/O Burst（I/O 突发）
### Child Process / 子进程
- **中文**：子进程
...
### CPU Burst / CPU 突发
- **中文**：CPU 突发
- **定义**：进程连续执行计算而不进行 I/O 的时间段
- **说明**：CPU 密集型进程有较长的 CPU 突发；I/O 密集型进程有较短的 CPU 突发
- **相关**：I/O Burst（I/O 突发）、Burst Time（突发时间）
...
- **音标**：/dɪˈspætʃ/
- **中文**：分派
- **定义**：调度器选择下一个要执行的进程并将其加载到 CPU 上的操作
- **说明**：分派程序（Dispatcher）负责上下文切换和切换到用户态
- **相关**：Dispatcher（分派程序）、Scheduler（调度器）
### Dispatcher / 分派程序
- **中文**：分派程序
- **定义**：负责将 CPU 控制权交给调度器选中的进程的模块
- **说明**：分派程序需要尽可能快，因为每次进程切换都要调用它
- **相关**：Dispatch（分派）、Context Switch（上下文切换）
### Exception / 异常
- **音标**：/ɪkˈsepʃn/
- **中文**：异常
- **定义**：程序执行过程中发生的同步事件，需要操作系统处理
- **说明**：异常包括除零、页错误（Page Fault）、段错误（Segmentation Fault）等；与中断不同，异常是同步的
- **相关**：Interrupt（中断）、Trap（陷入）、Page Fault（页错误）
### Exec / 执行（exec）
- **音标**：/ɪɡˈzek/
- **中文**：执行（exec）
...
- **音标**：/ˈfaɪfoʊ/
- **中文**：先进先出（命名管道）
- **定义**：Unix 系统中的命名管道，按先进先出顺序传输数据
- **说明**：FIFO 在文件系统中有一个路径名，允许无关的进程打开和读写
- **相关**：Named Pipe（命名管道）、Queue（队列）
...
### Hard Real-Time / 硬实时
- **中文**：硬实时
- **定义**：必须保证任务在截止时间前完成的实时系统
- **说明**：错过截止时间会导致严重后果；用于航空航天、医疗设备等关键系统
- **相关**：Soft Real-Time（软实时）、Deadline（截止时间）
### I/O Burst / I/O 突发
- **中文**：I/O 突发
- **定义**：进程进行 I/O 操作而不使用 CPU 的时间段
- **说明**：I/O 密集型进程有较长的 I/O 突发；良好的调度算法应该优先调度 I/O 密集型进程以提高资源利用率
- **相关**：CPU Burst（CPU 突发）、Burst Time（突发时间）
### Init Process / 初始化进程
- **中文**：初始化进程
- **定义**：系统启动时创建的第一个进程（PID 1），是所有用户进程的祖先
- **说明**：init 进程负责启动系统服务、收养孤儿进程；现代 Linux 中通常被 systemd 替代
- **相关**：Systemd、Process Tree（进程树）、Orphan Process（孤儿进程）
...
- **中文**：中断处理程序
- **定义**：操作系统中处理特定中断的函数
- **说明**：中断处理程序需要快速执行，通常只完成必要的工作，将复杂处理推迟到下半部（Bottom Half）
- **相关**：Interrupt（中断）、Top Half（上半部）、Bottom Half（下半部）
### Job / 作业
- **音标**：/dʒɑːb/
- **中文**：作业
- **定义**：用户提交给计算机系统的一个计算任务单元
- **说明**：作业可以包含多个进程，是批处理系统中的概念；从提交到完成经过提交、后备、执行、完成四个状态
- **相关**：Batch Processing（批处理）、Job Scheduler（作业调度器）
...
- **中文**：轻量级进程
- **定义**：内核支持的线程实现，是用户线程和内核线程之间的中间层
- **说明**：LWP 由内核调度，可以支持多个用户线程；每个 LWP 对应一个内核线程
- **相关**：Thread（线程）、Kernel Thread（内核线程）、User Thread（用户线程）
### Mailbox / 邮箱
- **中文**：邮箱
- **定义**：进程间通过发送和接收消息进行通信的抽象机制
- **说明**：每个进程可以有多个邮箱；消息发送到接收进程的邮箱中
- **相关**：Message Queue（消息队列）、Message Passing（消息传递）
### Main Thread / 主线程
- **中文**：主线程
- **定义**：进程创建时的第一个线程，执行 main 函数
- **说明**：主线程通常负责初始化、创建其他线程、等待其他线程结束
- **相关**：Thread（线程）、Process（进程）
### Memory Mapping / 内存映射
- **中文**：内存映射
- **定义**：将文件或设备映射到进程的地址空间的技术
- **说明**：mmap() 系统调用实现内存映射；映射后，对内存的读写相当于对文件的读写
- **相关**：Shared Memory（共享内存）、Virtual Memory（虚拟内存）
### Message Passing / 消息传递
- **中文**：消息传递
- **定义**：进程通过发送和接收消息进行通信的范式
- **说明**：可以是阻塞（同步）或非阻塞（异步）的；常用于分布式系统中
- **相关**：Shared Memory（共享内存）、RPC（远程过程调用）
...
### Named Pipe / 命名管道
- **中文**：命名管道
- **定义**：具有文件系统路径名的管道，可用于无关进程间通信
- **说明**：在 Unix 中称为 FIFO（First In First Out）；在 Windows 中称为 Named Pipe
- **相关**：FIFO、Pipe（管道）
### New / 新建
- **中文**：新建状态
- **定义**：进程刚被创建，尚未被调度执行
- **说明**：操作系统为新进程分配 PCB 和必要的资源
- **相关**：Process Creation（进程创建）
...
### Parent Process Identifier (PPID) / 父进程标识符
- **中文**：父进程标识符
- **定义**：创建当前进程的父进程的 PID
- **相关**：PID（进程标识符）、Process Tree（进程树）
...
### Process Group / 进程组
- **中文**：进程组
- **定义**：一个或多个进程的集合，通常与同一个作业相关联
- **说明**：进程组有一个组长（Group Leader），组长的 PID 就是进程组 ID（PGID）；信号可以发送给整个进程组
- **相关**：Session（会话）、Job Control（作业控制）
...
### Process Tree / 进程树
- **中文**：进程树
- **定义**：系统中所有进程按父子关系组织的树形结构
- **说明**：根节点是 init 进程（PID 1）；pstree 命令可以查看进程树
- **相关**：Parent Process（父进程）、Child Process（子进程）、Init Process（初始化进程）
### Program / 程序
- **音标**：/ˈproʊɡræm/
- **中文**：程序
- **定义**：存储在磁盘上的可执行文件，是指令和数据的集合
- **说明**：程序是静态的，不包含执行状态；一个程序可以对应多个进程
- **相关**：Process（进程）、Executable（可执行文件）、Binary（二进制文件）
### Race Condition / 竞态条件
- **中文**：竞态条件
...
- **中文**：实时系统
- **定义**：需要在严格时间限制内完成任务的操作系统
- **说明**：分为硬实时（Hard Real-Time，必须满足截止时间）和软实时（Soft Real-Time，尽量满足截止时间）
- **相关**：Deadline（截止时间）、Response Time（响应时间）、RTOS（实时操作系统）
### Reentrant / 可重入
- **音标**：/riːˈentrənt/
- **中文**：可重入
- **定义**：函数可以被中断并在中断处理完成后再次安全调用
- **说明**：可重入函数不使用全局变量或静态变量；是可重入的函数一定是线程安全的，反之不一定
- **相关**：Thread-Safe（线程安全）、Recursive（递归）
### Response Time / 响应时间
- **中文**：响应时间
- **定义**：从提交请求到系统首次响应的时间
...
- **音标**：/ˈsɜːrvɪs/
- **中文**：服务
- **定义**：在后台运行的系统功能模块，通常以守护进程形式存在
- **说明**：如 Web 服务、数据库服务、打印服务等；systemd 使用 service 单元管理服务
- **相关**：Daemon（守护进程）、Systemd、Server（服务器）
### Session / 会话
- **中文**：会话
- **定义**：一个或多个进程组的集合
- **说明**：会话通常对应一个用户登录；会话首进程（Session Leader）是会话的创建者
- **相关**：Process Group（进程组）、Terminal（终端）、Daemon（守护进程）
### Session Leader / 会话首进程
- **中文**：会话首进程
- **定义**：创建会话的进程，通常是用户登录 Shell
- **说明**：会话首进程控制终端；守护进程需要脱离会话以避免被终端信号影响
- **相关**：Session（会话）、Process Group（进程组）
### Shared Memory / 共享内存
- **中文**：共享内存
...
- **中文**：信号处理函数
- **定义**：进程接收到信号时调用的函数
- **说明**：可以自定义信号处理函数，或使用默认处理（终止、忽略、核心转储等）
- **相关**：Signal（信号）、Callback（回调函数）
### Socket / 套接字
- **音标**：/ˈsɑːkɪt/
- **中文**：套接字
...
### Soft Real-Time / 软实时
- **中文**：软实时
- **定义**：尽量保证任务在截止时间前完成，但偶尔错过可接受的实时系统
- **说明**：用于多媒体、游戏等对延迟敏感但非关键的系统
- **相关**：Hard Real-Time（硬实时）、Deadline（截止时间）
### Spawn / 生成
- **音标**：/spɔːn/
- **中文**：生成
- **定义**：创建并启动新进程的操作
- **说明**：在 Windows 系统中常用；spawn 通常组合了 fork 和 exec 的功能
- **相关**：Fork（创建）、Exec（执行）、CreateProcess（创建进程）
### Suspended / 挂起
- **音标**：/səˈspendɪd/
- **中文**：挂起状态
- **定义**：进程被换出内存，存放到外存中
- **说明**：挂起分为就绪挂起（Ready Suspended）和阻塞挂起（Blocked Suspended）；用于内存紧张时释放内存
- **相关**：Swap Out（换出）、Swap In（换入）、Virtual Memory（虚拟内存）
### System Call / 系统调用
- **中文**：系统调用
...
### Systemd / 系统守护进程管理器
- **中文**：系统守护进程管理器
- **定义**：Linux 系统的初始化系统和服务管理器
- **说明**：systemd 替代了传统的 init 系统，负责启动系统服务、管理守护进程、处理依赖关系
- **相关**：Init Process（初始化进程）、Daemon（守护进程）、Service（服务）
### Task / 任务
- **音标**：/tæsk/
- **中文**：任务
- **定义**：一般与进程同义，在某些系统中指代调度单元
- **说明**：在实时系统中，任务通常指具有截止时间要求的进程
- **相关**：Process（进程）、Job（作业）
### Terminate / 终止
- **音标**：/ˈtɜːrmɪneɪt/
- **中文**：终止
- **定义**：结束进程的执行
- **说明**：可以是正常终止（如调用 exit()）或异常终止（如被信号杀死）
- **相关**：Exit（退出）、Kill（杀死）、Abort（中止）
### Terminated / 终止
- **音标**：/ˈtɜːrmɪneɪtɪd/
- **中文**：终止状态
- **定义**：进程执行完毕或因错误被操作系统终止
- **说明**：操作系统回收进程占用的资源，但 PCB 可能暂时保留供父进程收集状态信息
- **相关**：Exit（退出）、Zombie Process（僵尸进程）、Orphan Process（孤儿进程）
### Thread / 线程
- **音标**：/θred/
- **中文**：线程
...
### Thread Pool / 线程池
- **中文**：线程池
- **定义**：预先创建的一组线程，用于处理提交的任务
- **说明**：避免频繁创建和销毁线程的开销；线程池中的线程可以复用
- **相关**：Worker Thread（工作线程）、Task Queue（任务队列）
### Thread-Safe / 线程安全
- **中文**：线程安全
...
- **音标**：/træp/
- **中文**：陷入（自陷）
- **定义**：程序执行时主动触发的中断，用于进入内核态
- **说明**：系统调用通过陷入指令（如 int 0x80、syscall）实现；与硬件中断不同，陷入是同步的
- **相关**：System Call（系统调用）、Interrupt（中断）、Exception（异常）
